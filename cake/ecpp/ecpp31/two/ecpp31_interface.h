#ifndef PERSON_INTERFACES
#define PERSON_INTERFACES
...
// interfaces class / handle class, abstruct class

class Person
{
public:
    static boost::shared_ptr<Person> create(const std::string& name, 
                                            const Data& birthday, 
                                            const Address& addr);       

    virtual ~Person() {};// 必须定义
    virtual std::string name() const = 0;
    virtual Data birthDate()  const  = 0;
    virtual Address address()  const = 0;
};

#endif //PERSON_INTERFACES
